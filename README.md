...
```powershell
# 方式一：官方安装包
# 下载 https://nodejs.org/ 安装 LTS 版（>= 22.12.0），安装过程勾选 "Automatically install the necessary tools"
...
后端使用 **[uv](https://docs.astral.sh/uv/)** 管理 Python 虚拟环境与依赖，无需单独安装 Python。
...
```powershell
# 方式一：官方安装包
# 下载 https://www.postgresql.org/download/windows/ 安装，记住设置的 postgres 用户密码

# 方式二：使用 winget
winget install PostgreSQL.PostgreSQL.17

# 方式三：使用 Docker
docker run -d --name lkm-postgres -p 5432:5432 ^
  -e POSTGRES_USER=lkm -e POSTGRES_PASSWORD=lkm123 -e POSTGRES_DB=lkm ^
  postgres:17-alpine
```

安装后将 PostgreSQL 的 `bin` 目录加入 PATH（默认 `C:\Program Files\PostgreSQL\17\bin`）。
...
# 创建数据库用户和数据库
sudo -u postgres createuser lkm -P       # 输入密码: lkm123
sudo -u postgres createdb lkm -O lkm
```
...
> 环境变量 `PG_HOST`、`PG_PORT` 等均已提供默认值（见下方表格），本地开发无需额外配置。

---

## 部署流程

提供两种方式：**一键脚本**（推荐）和**手动逐步部署**。

### 方式一：一键脚本

项目内置了自动完成环境检测 → 依赖安装 → 数据库准备 → 启动服务的脚本。
...
> 生产脚本构建前端静态文件到 `dist/`，自动检测 nginx 或退回 Python HTTP server 托管。
...
```bash
# Linux (作为 postgres 用户执行)
sudo -u postgres createuser lkm -P
sudo -u postgres createdb lkm -O lkm
```

```bat
:: Windows (用 psql 执行)
psql -U postgres -c "CREATE USER lkm WITH PASSWORD 'lkm123';"
psql -U postgres -c "CREATE DATABASE lkm OWNER lkm;"
```
...
```shell
# 终端 1：启动前端 (http://localhost:4321)
pnpm run dev

# 终端 2：启动后端 (http://localhost:8000)
...
```shell
pnpm run build                    # 构建前端静态文件到 dist/
uv run uvicorn backend.main:app --host 0.0.0.0 --port 8000  # 生产启动后端
```

将 `dist/` 目录部署到任意静态托管服务（Vercel、Netlify、Nginx 等），后端单独部署。

---

### 环境变量参考

所有环境变量均可选，已提供本地开发默认值：
...
| `uv pip install -r backend/requirements.txt` | 安装依赖             |
| `uv run uvicorn backend.main:app --reload`   | 启动开发服务器       |
| `uv run python backend/test_db.py`           | 测试数据库连接       |
...
uv run uvicorn backend.main:app --host 0.0.0.0 --port 8000  # 启动后端
```
