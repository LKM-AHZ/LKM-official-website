...
const DataRainBackground = ({ fontSize = 18, fontFamily = 'Space Grotesk', color: propColor, rippleColor: propRippleColor, density = 0.05, className = '', flickerSpeed = 0.05, trailLength = 15, charSet = ['0', '1', 'あ', 'ｑ', 'Æ', 'Ψ'], charChangeRate = 0.1, deflectionForce = 50, mouseGlowRadius = 80, rippleGrowthRate = 3, rippleFadeRate = 0.015, rippleLineWidth = 2, charSpeedMin = 0.5, charSpeedMax = 2.5 }) => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            columnsRef.current.forEach(column => {
                var _a;
                const { x: _x, originalX, yPositions, chars, speeds, deflectionX, deflectionDecay } = column;
                let columnChanged = false;
                for (let i = 0; i < yPositions.length; i++) {
                    // Move character down
                    yPositions[i] += speeds[i];
                    // Apply deflection physics
                    const distToMouse = Math.sqrt(Math.pow(originalX + deflectionX[i] - mouseRef.current.x, 2) +
                    ...
                    if (distToMouse < deflectionForce && mouseRef.current.intensity > 0.2) {
                        const angle = Math.atan2(yPositions[i] - mouseRef.current.y, originalX + deflectionX[i] - mouseRef.current.x);
                    ...
                    const [r, g, b] = ((_a = color.match(/\d+/g)) === null || _a === void 0 ? void 0 : _a.map(Number)) || [0, 255, 0];
